...
?>
<form method="post" action="<?=$config[main]?>?action=msgadmin">
<table class="inputtable">
<tr><th colspan="3"><input type="checkbox" name="msgs_timelimit" value="1"<?if ($msgs_timelimit) print " checked";?>> Display messages between <input type="text" name="msgs_age1" value="<?=$minage?>" size="3"> and <input type="text" name="msgs_age2" value="<?=$maxage?>" size="3"> hours ago</th></tr>
<tr><th>View messages sent from empire</th>
    <td>#<input type="text" name="msgs_from" size="4" value="<?=$msgs_from?>"></td>
    <th><input type="submit" name="msgs_doviewfrom" value="View"></th></tr>
<tr><th>View messages sent to empire</th>
    <td>#<input type="text" name="msgs_to" size="4" value="<?=$msgs_to?>"></td>
    <th><input type="submit" name="msgs_doviewto" value="View"></th></tr>
<tr><th>View messages sent between empires</th>
...
$msgs = mysql_query("SELECT time,src as num_s,p1.empire as name_s,dest as num_d,p2.empire as name_d,msg FROM $messagedb ".
			"LEFT JOIN $playerdb AS p1 ON (src=p1.num) ".
			"LEFT JOIN $playerdb AS p2 ON (dest=p2.num) ".
...
<tr><td><?=date("Y/m/d H:i:s",$message[time])?></td>
    <td><?=$message[name_s]?> (#<?=$message[num_s]?>)</td>
    <td><?=$message[name_d]?> (#<?=$message[num_d]?>)</td>
    <td style="text-align:left"><?printmsg(str_replace("\r","",str_replace("\n","<br> ",$message[msg])))?></td></tr>
<?
}
?>
</table>
